...
var Board = function () {
  ...
  _createClass(Board, [{
    ...
    key: "addRobot",
    value: function addRobot(x, y, orientation) {
      var robotName = "robot" + this.currentRobotIndex++;
      var board = {
      ...
      var robot = new _robot2.default(board, robotName, x, y, orientation);
    ...
    key: "moveRobot",
    value: function moveRobot(robotName, movements) {

      var robot = this.robots[robotName];
      var output = robot.moveAroundBoard(movements);
...
},{"./robot":300}],299:[function(require,module,exports){
"use strict";
  ...
  _createClass(App, [{
    ...
    value: function parseInput(input) {
      try {
        ...
        var gridWidth = parseInt(gridSize[0], 10);
        var gridHeight = parseInt(gridSize[1], 10);
        ...
        this.board = new _board2.default(document.getElementById('board'), gridWidth, gridHeight);

        //Setup the robots
        var newRobotIndex = 0;
        var currentRobotName = void 0;
        var robotOutput = [];
        for (var lineIndex = 1; lineIndex < lines.length; lineIndex++) {
          ...
          if (line === '') {
            ...
            var robotX = parseInt(robotPositon[0], 10);
            var robotY = parseInt(robotPositon[1], 10);
            var robotOrientation = robotPositon[2];
    ...
    value: function printOutput(output) {
      ...
      var outputString = "<h2>Output</h2>";
      if (output.length === 0) {
        outputString += '<p>Nothing to see here</p>';
      } else {
        outputString += output.map(function (text) {
          return "<p>" + text + "</p>";
        }).join(' ');
      }
...
},{"./board":298,"babel-polyfill":1}],300:[function(require,module,exports){
"use strict";
...
var rotateLeft = -90;
var rotateRight = 90;
var moveX = 50; //each square is 50px;
var moveY = -50; // each square is 50px; Y axis is in the opposite direction

var Robot = function () {
  function Robot(board, name) {
    var x = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
    var y = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 0;
    var orientation = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 'N';
  ...
  _createClass(Robot, [{
    key: "setLostScents",
    value: function setLostScents(lostScents) {
    ...
    key: "moveToSquare",
    value: function moveToSquare(x, y) {
    ...
    key: "moveAroundBoard",
    value: function moveAroundBoard(movements) {
      var _this = this;

      //Accept a string of movements for the robot
      movements.split('').every(function (move) {
        var newX = _this.currentX,
            newY = _this.currentY;
        switch (move.toUpperCase()) {
          case 'L':
            //rotate left  
            _this.currentRotate += rotateLeft;
            break;
          case 'R':
            //rotate right  
            _this.currentRotate += rotateRight;
            break;
          case 'F':
            //forward (depending on direction)
            switch (_this.currentRotate) {
              case 0:
                newY += 1;
                break;
              case 90:
        ...
        if (newX < 0 || newX > _this.board.width || newY < 0 || newY > _this.board.height) {
          //If some other robot got lost at this coordinate, ignore the move
          if (_this.lostScents["" + _this.currentX + _this.currentY] === undefined) {
            _this.isLost = true;
            // We're lost. Hide the robot and exit out of the entire loop
            _this.robot.style.display = 'none';
            return false;
          }
        } else {
          _this.currentX = newX;
        ...
        if (_this.currentRotate === 360) {
          _this.currentRotate = 0;
        } else if (_this.currentRotate < 0) {
          _this.currentRotate += 360;
        }
    ...
    value: function currentLocation() {
      ...
      var location = this.currentX + " " + this.currentY + " " + this._degreesToOrientation(this.currentRotate);
      if (this.isLost) {
        location += ' LOST';
      }
      return location;
    ...
    key: "_orientationToDegrees",
    value: function _orientationToDegrees(orientation) {
      switch (orientation) {
        case 'N':
          return 0;
        case 'E':
    ...
    key: "_degreesToOrientation",
    value: function _degreesToOrientation(degrees) {
      switch (degrees) {
        case 0:
          return 'N';
        case 90:
